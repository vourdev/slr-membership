...
- **Discounts member DTO** (`GET /discounts/` + `/{id}`) — ✅ `code` + `terms` now returned (member card shows the real copy-code + terms; placeholders auto-swapped). ⚠️ **still missing `value_label`** (short offer badge) → card shows a `Member offer` placeholder until the backend adds it.
