    ...
    React.useEffect(() => {
        ...
        if (el)
            setThemeClass(
                el.className
                    .split(' ')
                    .filter((c) => c === 'slr-member' || c === 'slr-admin' || c === 'dark' || c === 'dashboard-theme')
                    .join(' ')
            );
    }, []);
    ...
    return (
        <DialogPortal data-slot='dialog-portal'>
            ...
            <DialogPrimitive.Content
                ...
                {...props}>
                {children}
                <DialogPrimitive.Close className="ring-offset-background focus:ring-ring bg-background/80 text-foreground border-border absolute top-4 right-4 z-10 flex size-7 items-center justify-center rounded-full border opacity-90 backdrop-blur-sm transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4">
                    <XIcon />
