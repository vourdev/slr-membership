...
- Gold gradient (CTAs, brand text): `linear-gradient(89.12deg, #F5D78E 3.07%, #D4AF37 41.36%, #FFE066 60.5%, #A07018 98.79%)`
...
- Body text muted: `#CDCECF` · ultra-muted: `#8EA0B8` · gold accent label: `#E2B42B`
...
| Eyebrow label | `text-xs md:text-sm font-semibold uppercase text-[#E2B42B]` (gold) or `text-red-600` | Flanked by gradient hairline dividers |
| Body | `text-sm md:text-base text-[#CDCECF]` | |
| Micro/meta | `text-[10px] sm:text-xs uppercase tracking-widest text-[#8EA0B8]` | |

**Brand-text gradient pattern** (apply gold gradient to text):
```tsx
<span className="bg-[linear-gradient(89.12deg,#F5D78E_3.07%,#D4AF37_41.36%,#FFE066_60.5%,#A07018_98.79%)] bg-clip-text text-transparent">REWARDS</span>
```
...
<div className="relative isolate rounded-2xl p-1.25">
  <div className="absolute inset-0 -z-10 rounded-2xl bg-[linear-gradient(...)] [mask:linear-gradient(#fff_0_0)_content-box,linear-gradient(#fff_0_0)] mask-exclude" />
...
<Button
  ...
  style={{
    ...
    boxShadow: '0px 8px 12px 0px #FFFFFF14 inset, 0px 24px 24px -16px #DAAF1166, 16px 24px 64px -24px #FFFFFF14 inset, 0px 12px 24px 4px #A0701840'
  }}>
  Join Now
</Button>
```
...
<div className="flex items-center justify-center gap-2">
  ...
  <p className="text-xs md:text-sm font-semibold uppercase text-[#E2B42B]">Member Benefits</p>
...
**Form inputs:** Use shadcn `Input`/`Form` primitives; for dark surfaces, override with `bg-slr-navy-card border-slr-navy-border text-white placeholder:text-[#8EA0B8]`. Reference [sign-up/components/register-form.tsx](src/app/(auth)/(routes)/sign-up/components/register-form.tsx).
...
- Don't use plain `text-white` for body copy — use `text-[#CDCECF]` or `text-white/60..90` to keep depth.
